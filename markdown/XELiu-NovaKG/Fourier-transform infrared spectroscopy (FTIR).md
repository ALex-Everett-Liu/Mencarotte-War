...
- # Insa
    - 荧飒光学 FTIR 测试报告
        - **Resolution Test**
            ...
            - 红外光谱分辨率 ([[resolution]]，以 △v 表示) 是指[分辨两条相邻吸收谱线]的能力，它是由仪器[干涉仪动镜]([[moveable mirror]])的[移动距离]决定的，根据干涉仪的[工作原理]，通过[光程差]的数学计算，分辨率[近似等于][最大光程差的倒数]，也就是[动镜移动有效距离]2倍的倒数，例如[一台仪器]的[动镜移动有效距离]为4cm，这台仪器的[最大分辨率]为0.125 cm-1。动镜移动有效距离越长，分辨率越高，分辨率的数值越小。用于一般分析时，选用 4cm^-1 即可。
